    ...
    if os.path.exists(rootdir+"/" + name):
        ...
        for i in range(0, len(_fs)):
            ...
            if(filename == _fs_filename):
                ...
                flag += 1
        return flag
        print("now falg is", flag)
        print(rootdir+"/" + name+"/"+fy_filename)
        if not os.path.exists(rootdir+"/" + name+"/"+fy_filename):
...
def fy_deploy(rootdir, commitdir, tmpname, modulename, moduleversion):
    ...
    if os.path.exists(rootdir+"/"):
        if os.path.exists(rootdir+"/" + name):
            print(rootdir+"/" + name+"/"+fy_file)
            if os.path.exists(rootdir+"/" + name+"/"+fy_file):
    ...
    if(falg_value == 1):
        copyfile(rootdir+"/"+name+"/"+fy_file,
                 rootdir+"/"+name+"/"+fy_file+".bak")
    tmpl_falg_value = 0
    if os.path.exists(commitdir+"/"):
        print("the commitdir is ",commitdir)
        if os.path.exists(commitdir+"/" + name):
            print(commitdir+"/" + name)
            print(tmpname)
            print(commitdir+"/" + name+"/"+tmpname)
            if os.path.exists(commitdir+"/" + name+"/"+tmpname):
                print(commitdir+"/" + name+"/"+tmpname)
                tmpl_falg_value = 1
    ...
    if(tmpl_falg_value == 1):
        try:
            ...
            copyfile(commitdir+"/"+name+"/"+tmpname,
                     rootdir+"/"+name+"/"+fy_file)
            print("hello world")
            ...
            subprocess.run(linkcmd, shell=True, check=True,
